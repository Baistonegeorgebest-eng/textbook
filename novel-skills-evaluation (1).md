...
> 以下内容基于 2025-2026 年最新行业动态、学术研究和竞品分析补充。

### 补1：番茄/起点的 AI 检测已进入"动态对抗"阶段

**2025年2-3月的"AI清洗"事件**：

- 2025年2月底，番茄大批作者流量锐减、疑似被软封禁。平台客服理由为"水文"，但作者普遍认为是针对 AI 写作的清洗
- 番茄首秀新书在 2025年3月1日达到惊人的 **5000本/天**，其中大量 AI 辅助内容涌入流量池
- LOFTER 用**朱雀大模型**检测，将一位《哪吒2》同人写手标记为 AI 生成内容占比 **100%**

**各平台的分级管控（2025年）**：

| 平台 | 策略 | 具体标准 |
|------|------|---------|
| 晋江 | 最严格 | 2025年2月17日发布AI写作规范，按校对/描写/叙事/要素/粗纲/细纲6级分类，**只接受校对级别和粗纲级别**AI辅助 |
| 番茄 | 暗中管控 | 不公开标准，以"水文"为由软封禁。AI写作工具自己发的，但用多了又会被打 |
| 阅文 | 有条件的拥抱 | 发布"阅文妙笔"AI工具，但编辑普遍拒收明显AI稿件 |

**关键情报——阅文副总编辑胡说的判断**：
> "尤其DeepSeek的风格还是挺明显的，有一种强烈的**赛博科幻和视觉化、克系写作风格**，很难校正。如果没有相应剧情支撑，看起来就**言之无物**，会让小说更空洞、更水。"

**对 skill 体系的启示**：
1. 当前 skill 的"去AI味"主要针对**通用AI特征**（长句、感叹号、感官偏科），但**DeepSeek 有独特的风格指纹**需要专门处理
2. 平台检测已从"标点指纹"升级到**大模型检测**（朱雀），单纯的统计特征调整可能不够
3. 番茄的"水文"判定标准暗示：**过度润色 = 空洞**，比"有AI味"更危险

### 补2：AIGC检测技术的三大学术前沿

**前沿1：Perplexity + Burstiness 检测（2025主流）**

AI 生成文本的核心特征：
- **低困惑度（Perplexity）**：AI 倾向于选择高概率词，人类写作更"出人意料"
- **低突发性（Burstiness）**：AI 的句子长度和复杂度更均匀，人类写作有"爆发"和"平淡"的交替
- **高熵均匀性**：AI 的词汇选择分布更均匀，人类有更强的偏好集中

> arxiv 2501.03940 (2025.7) 提出 Perplexity Attention Weighted Networks，按 token 级别加权困惑度，检测精度提升 15%+

**对 skill 体系的启示**：
- 人味协议的"滑动窗口"和"风格轮转"天然对抗 burstiness 检测（制造不均匀性）
- 但"废话语保底"和"语言失败保底"可能反而增加困惑度（低概率词），这是正向的
- **建议增加"词汇惊喜度"指标**：每章至少3个低概率但合理的词汇选择

**前沿2：风格感知检测（EMNLP 2025 MoSEs）**

MoSEs 提出**动态阈值 + 风格建模**：
- 不是用固定阈值判断"AI vs 人类"，而是先建模文本的风格特征，再在该风格子空间内判断
- 意味着：**你的文风越独特，检测越难**（因为没有足够的同风格训练数据）

**对 skill 体系的启示**：
- 人味协议的"第三层：作者人味"（像你写的）不仅是审美追求，还是**技术护城河**
- 越个性化的写作风格，越难被 AIGC 检测器捕获
- **建议强化 expert-style-learner 的学习深度**：从用户修改中提取更细粒度的风格特征

**前沿3：多样性检测（arxiv 2509.18880, 2025.9）**

"Diversity Boosts AI-Generated Text Detection" 发现：
- AI 生成文本的**句间多样性**显著低于人类
- 检测器不仅看单句特征，还看**连续句子之间的变化幅度**

**对 skill 体系的启示**：
- 当前 skill 的"禁连续3句同句式"规则方向正确，但力度可能不够
- **建议增加"句间变化率"指标**：连续5句之间的主语、谓语结构、句长的变异系数

### 补3：竞品分析——Sudowrite 的做法

Sudowrite 是海外最流行的 AI 小说写作工具（基于 GPT-3/4），它的功能设计有几个值得借鉴的点：

| Sudowrite 功能 | 对标小说 skill | 借鉴价值 |
|---------------|--------------|---------|
| **Story Engine** | novel-volume-workflow | Sudowrite 的 Story Engine 是"输入大纲→自动生成章节"的全流程引擎，但**允许用户在每个步骤介入修改**。当前 skill 的自动化程度更高但用户介入点更少 |
| **Canvas（故事板）** | 无直接对标 | 可视化的故事结构板，拖拽调整章节顺序、角色弧光、情感曲线。当前 skill 纯文本操作 |
| **Brainstorm** | expert-plot-shuangdian | 生成角色/情节/世界观的多个备选方案让用户选择，而非直接给出"最优解" |
| **Describe / Expand** | expert-sensory-prose | 选中一段文字后自动生成感官描写扩充。**用户先有骨架，AI填肉**——和种子写作法一致 |
| **Rewrite / Shrink Ray** | expert-revision | 多种改写风格（更生动/更简洁/更对话化等），用户选择方向 |
| **Character Bible** | expert-character-dna | 自动维护角色一致性文档，每次生成新内容时检查是否符合角色设定 |

**关键差异**：Sudowrite 的设计哲学是"**AI 给选项，人类做选择**"，而当前小说 skill 的设计是"**AI 直接输出最终结果**"。

**对 skill 体系的启示**：
1. **增加"分支生成"模式**：关键场景（开篇、高潮、转折）生成 2-3 个版本让用户选择，而非直接输出一个版本
2. **增加可视化故事板**：将 PLOT-FRAMEWORK.md 以结构化的方式呈现，支持调整
3. **强化 Character Bible**：将 expert-character-dna 的输出做成可维护的文档，每次创作时自动校验

### 补4：行业共识——"AI 只能在作者理解的范围内辅助"

阅文副总编辑胡说的核心观点：
> "AI只能在作者理解的范围内辅助作者。作者不理解的知识没法用AI拼凑。"

> "AI是用逻辑思考的，但人没那么有逻辑。家国大事背后也许是很儿戏的东西，文学有时候是反逻辑反常识的。"

这两个判断精确地指出了当前 skill 体系的理论天花板：

1. **逻辑 vs 反逻辑**：skill 体系的所有规则都是"逻辑"的（硬约束→滑动窗口→质检），但好的文学有时需要"反逻辑"——这正是人味协议中"扰动机制"的价值，但当前扰动规则还是偏"逻辑化"（20%概率做X、30%概率做Y）
2. **理解范围内**：AI 能执行规则，但不能"理解"为什么要违反规则。种子写作法的核心价值就在这里——人类提供"反逻辑"的种子，AI 在规则内展开

**对 skill 体系的启示**：
- 扰动机制应该更"无逻辑"：不只是概率触发预定义的行为，而是偶尔**完全跳过**某些规则
- 增加"反规则章节"：每10-20章允许有1章完全不执行人味硬约束，让风格自然流动
...
_评估日期：2026-05-07_
_数据来源：novel-skills-v1.18.1.tar.gz + public-transfer 仓库 + 联网研究（2025-2026行业动态/学术论文/竞品分析）_
